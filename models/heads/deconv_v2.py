...
class Conv1x1(Module):
    def __init__(self, num_in, num_out, use_bias=True):
        ...
        self.block = Sequential(
            Conv2d(num_in, num_out, kernel_size=1, bias=use_bias), 
            BatchNorm2d(num_out),
            ReLU(True)
        )
...
class Conv3x3(Module):
    def __init__(self, num_in, num_out, kernel_size=3, padding=1, use_bias=True):
        super().__init__()
        self.block = Sequential(
            Conv2d(num_in, num_out, kernel_size=kernel_size, padding=padding, bias=use_bias), 
            BatchNorm2d(num_out), 
            ReLU(True)
        )
...
class DynamicConv(Module):
    def __init__(self, in_channels, out_channels, kernel_sizes, stride = 1, use_bias=True) -> None:
        ...
        self.atten = Sequential(*[
            ...
            Flatten(),
            Linear(in_features=in_channels, out_features=max(in_channels, in_channels//4)),
            ReLU(True),
            Linear(in_features=max(in_channels, in_channels//4), out_features=len(kernel_sizes)),
            Softmax()
        ])

        self.convs = ModuleList([
            Conv2d(in_channels, out_channels, kernel_size=k, stride=stride, padding='same', bias=use_bias)
    ...
    def forward(self, x):
        ...
        weights = self.atten(x)#.view(-1, len(self.kernel_sizes))
        # print("weights", weights.shape)
        conv_outs = None
        for i, c in enumerate(self.convs):
            conv_out = c(x)
            # print(i, "conv_out", conv_out.shape)
            ...
            if i == 0:
                ...
                conv_outs += conv_out_weighted
        # print("conv_outs", conv_outs.shape)
        return conv_outs
...
class SPPM(Module):
    def __init__(self, in_channels, inter_channels, out_channels, pool_sizes=[1, 2, 4], output_size_ratio = 1) -> None:
        super(SPPM, self).__init__()
        self.output_size_ratio = output_size_ratio
    ...
    def forward(self, x):
        ...
        for i in range(len(self.poolings)):
            p = self.poolings[i](x)
            if i == 0:
                pooled = functional.interpolate(p, size=(height // self.output_size_ratio, width // self.output_size_ratio), mode='bilinear')
            else:
                pooled += functional.interpolate(p, size=(height // self.output_size_ratio, width // self.output_size_ratio), mode='bilinear')
...
class FusionModule(Module):
    ...
    def avg_max_reduce_channel(self, x):
        ...
        # print("mean_value", mean_value.shape, "max_value", max_value.shape)
    ...
    def fuse(self, x, y):
        ...
        # print("atten logits", atten.shape)
        atten = functional.sigmoid(atten)
    ...
    def forward(self, x, y):
        # print("forward", x.shape, y.shape, self.x_ch, self.y_ch)
...
class DeconvV2(Module):
    def __init__(self, resnet_module, in_channels=[256, 512, 1024, 2048], outplanes=1024, kernel_sizes = [3, 5], output_logits=True):
        ...
        self.sppm = SPPM(in_channels[-1], inter_channels=in_channels[-1], out_channels=in_channels[-1], output_size_ratio=2)
        self.fuses = ModuleList()

        for i in range(len(in_channels)):
            # in_high_ch = in_channels[i+1] if i < len(in_channels) - 1 else in_channels[-1]
            in_low_ch = in_channels[i]
            in_high_ch = in_low_ch
            # print(i, f"x_ch={in_high_ch}; y_ch={in_low_ch}; out_ch={in_low_ch // 2}")
        ...
        final_out_ch = in_channels[0] // 2
        self.logits = Sequential(*[
            DynamicConv( in_channels=final_out_ch, out_channels=(final_out_ch//2) * 16, kernel_sizes=kernel_sizes),
            PixelShuffle(4),
            BatchNorm2d(final_out_ch//2),
            ReLU(inplace=True),
            ...
            Shortcut(Sequential(*[
                DynamicConv( in_channels=final_out_ch//2, out_channels=final_out_ch//2, kernel_sizes=kernel_sizes),
                BatchNorm2d( final_out_ch//2 ),
                ReLU(inplace=True),
            ...
            DynamicConv( in_channels=final_out_ch//2, out_channels=1, kernel_sizes=kernel_sizes),
    ...
    def forward(self, x):
        ...
        for i in reversed(range(len(self.in_channels))):
            if i == len(self.in_channels) - 1:
                merged.append(self.fuses[i]( features[i], sppm_out ))
            else:
                merged.append(self.fuses[i]( features[i], merged[-1] ))
        ...
        logits = merged[-1] #upscaled[-1]
        # print("logits", logits.shape)
        logits = self.logits(logits)
        # logits = functional.interpolate(logits, scale_factor=4, mode='bilinear')
        # print("logits", logits.shape)
